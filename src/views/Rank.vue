        ...
        <ul>
          <li v-for="(value, key) in category.titles" :key="key">
            ...
            <ul class="normal-group" v-if="value === '官方榜'">
              <li v-for="(obj, index) in officialList" :key="obj.id" @click.stop="selectedItem(index)">
                <div class="rank-left">
                  <img v-lazy="obj.coverImgUrl + '?imageView=1&type=webp&thumbnail=369x0'" alt="">
                  <p>{{ obj.updateFrequency }}</p>
                </div>
                <div class="rank-right">
                  <p v-for="(song, index) in obj.tracks" :key="song.id">{{index + 1}}.{{song.al.name}}-{{song.ar[0].name}}</p>
                </div>
              </li>
            </ul>
...
<script>
import { getTopListDetail, getOfficialList } from '../api/index'
